...
// Naive matrix mul, but rather than the usual case where A x B will cause coalesced memory accesses for A and not for B, 
// we transpose B so those accesses are coalesced as well.
#include <stdio.h>
#include <stdlib.h>
...
__global__ void matrix_multiply(const int *const matrix_a, const int *const matrix_b, int *const matrix_sum_result)
{
    ...
    if (i < MATRIX_DIMENSION && j < MATRIX_DIMENSION)
    {
...
int main()
{
    ...
    for (int i = 0; i < MATRIX_DIMENSION; i++)
    {
        for (int j = 0; j < i; j++)
        {
            ...
            host_matrix_b[j * MATRIX_DIMENSION + i] = temp;
        }
    }

    printf("\npost transpose matrix B :: \n");
    for (int i = 0; i < MATRIX_DIMENSION; i++)
    {
        for (int j = 0; j < MATRIX_DIMENSION; j++)
        {
    ...
    const dim3 thread_group_dim(1, 1, 1);
