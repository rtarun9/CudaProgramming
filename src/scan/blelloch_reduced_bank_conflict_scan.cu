...
#define COMPUTE_CONFLICT_FREE_index(n) ((n) + CONFLICT_FREE_OFFSET((n)))
...
{
    // Since the conflict free offset(X) value is added to shared mem index, size of shared memory also increases.
    __shared__ int smem[BLOCK_DIM + NUM_BANKS];

    const int tx = threadIdx.x + blockIdx.x * blockDim.x;

    smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x)] = input_output_array[tx];
    __syncthreads();
    ...
    for (int i = 2; i < BLOCK_DIM; i *= 2)
    {
        ...
        {
            val_to_add_to_smem = smem[COMPUTE_CONFLICT_FREE_index((threadIdx.x - i / 2))];
        }

        smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x)] += val_to_add_to_smem;
        __syncthreads();
    }


    smem[COMPUTE_CONFLICT_FREE_index(blockDim.x - 1)] = 0;
    __syncthreads();
    ...
    for (int i = BLOCK_DIM; i > 0; i = i / 2)
    {
        ...
        {
            int left = smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x - i / 2)];
            int right = smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x)];

            smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x)] = left + right;
            smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x - i / 2)] = right;
        }
    ...
    {
        per_block_scan_output[blockIdx.x] = input_output_array[tx] + smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x)];
    }

    input_output_array[tx] = smem[COMPUTE_CONFLICT_FREE_index(threadIdx.x)];
}
